...
subtitle: "Réaliser une Classification Ascendante Hiérarchique avec R-base"
author: 
- Hugues pecout
date: last-modified
ordre: 01
...
Il ne reste plus qu'à découper le dendrogramme obtenu à une certaine hauteur pour obtenir une partition des individus. La qualité d'une partition se traduit mathématiquement par une petite variabilité intra-classe et une grande variabilité inter-classes. La forme et les sauts d'inertie du dendogramme permettent d'apprécier l'évolution de ces variabilités en fonction du nombre de partitions.
...
La fonction `cutree()` permet de récupérer la partition des individus dans le nombre de classes souhaitées. Elle renvoie un vecteur comportant la classe d'appartrenance de chaque individu. l'ordre des individus étant identique à celui du tableau de départ, la classe d'appartenance des individus peut facilement être ajoutée dans une nouvelle colonne.
...
#| code-fold: FALSE
library(gtsummary)
tbl_summary(var_quanti, by = CAH)
```

L'intérprétation des profil des classes peut également être facilitée par la représentation graphique. Commencez par standardiser les valeurs avec la fonction `scale()`
...
Regrouper ensuite les individus de chaque en calulant les moyennes (ou médiannes) de chaque indicateur pour l'ensemble des classes avec la fonction `aggregate()`.
...
Profil_classe <- aggregate(scaled_var[, -18],
                           by = list(cluster = scaled_var$CLASSE),
                           mean, na.rm = TRUE)

```

Il est nécessaire de tranformer le tableau en format long avec afin de le représenter graphiquement. Pour cela, vous pouvez utiliser la fonction `pivot_longer()` du package `tidyr`.
...
La library `ggplot2` est ensuite très efficace pour représenter graphiquement les profils de chaque classe.
...
ggplot(Profil_classe_lg) +
  ...
  scale_fill_manual(values = c("lightblue4", "red4", "yellow4", "blue4")) +
  facet_wrap(~cluster) +
  coord_flip() + theme_bw()
```
...
library(mapsf)
mf_map(x = pays_afrique, var = "CAH", type = "typo",
      pal = c("lightblue4", "red4", "yellow4", "blue4"),
      leg_title = "Clusters")
